...
summary: Esamina le considerazioni RBAC di ONTAP per gli ONTAP tools prima di usarli in un ambiente di produzione. 
---
= ONTAP RBAC negli ONTAP tools for VMware vSphere
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ONTAP tools for VMware vSphere supporta il controllo degli accessi in base al ruolo (RBAC) tramite ONTAP, permettendoti di creare utenti e ruoli personalizzati che limitano l'accesso alle risorse di storage. Capire il processo di configurazione e i livelli di privilegio ti aiuta a impostare controlli di accesso appropriati prima di implementare ONTAP tools in un ambiente di produzione.
...
La sezione seguente descrive a high-level le principali fasi di configurazione. Consulta link:../configure/configure-user-role-and-privileges.html["Configurare i ruoli e i privilegi degli utenti ONTAP"] per maggiori dettagli.
...
. Scorri verso il basso fino a *Utenti e ruoli* e seleziona  `-->`.
...
Dopo aver selezionato le opzioni di configurazione per il tuo ambiente, seleziona *Aggiungi* e ONTAP crea l'utente e il ruolo. Il nome del ruolo generato è una concatenazione dei seguenti valori:
...
Il nuovo utente appare nell'elenco nella pagina *Utenti e ruoli*. Nota che sono supportati entrambi i metodi di accesso utente, HTTP e ONTAPI.
