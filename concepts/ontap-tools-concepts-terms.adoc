...
summary: Esamina i concetti e i termini chiave utilizzati nella documentazione di ONTAP tools for VMware vSphere. 
---
= Termini chiave e definizioni per ONTAP tools
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Acquisisci familiarità con i concetti e la terminologia chiave utilizzati in ONTAP tools for VMware vSphere. Le definizioni riguardano storage backends, volumi virtuali (vVols), SnapMirror active sync, VASA, SRA e altri componenti principali.
...
Un namespace NVMe è una quantità di memoria non volatile che può essere formattata in blocchi logici. I namespace sono l'equivalente delle LUN per i protocolli FC e iSCSI, e un sottosistema NVMe è analogo a un igroup. Un sottosistema NVMe può essere associato a iniziatori, permettendo loro di accedere ai namespace all'interno del sottosistema.
...
SnapMirror active sync ti permette di continuare le operazioni di business anche in caso di guasto completo del sito, supportando il failover trasparente delle applicazioni tramite una copia secondaria. SnapMirror active sync non richiede interventi manuali o script personalizzati per attivare un failover. Scopri di più https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/index.html["Sincronizzazione attiva di SnapMirror"].
...
NOTE: SnapMirror active sync supporta l'accesso uniforme agli host per qualsiasi implementazione; solo le implementazioni simmetriche active/active supportano l'accesso non uniforme agli host. Scopri di più su  https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/index.html["Panoramica della sincronizzazione attiva di SnapMirror in ONTAP"].
...
vVols forniscono un'astrazione a livello di volume per lo storage utilizzato da una macchina virtuale. vVols includono diversi vantaggi e offrono un'alternativa all'utilizzo di un LUN tradizionale. Un vVol datastore è in genere associato a un singolo LUN che funge da container per vVols.
...
VMware Live Site Recovery, precedentemente noto come Site Recovery Manager (SRM), offre funzionalità di business continuity, disaster recovery, migrazione del sito e test non invasivi per gli ambienti virtuali VMware.
