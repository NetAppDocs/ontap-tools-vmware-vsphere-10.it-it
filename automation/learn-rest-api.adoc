...
ONTAP tools for VMware vSphere 10 espone un singolo indirizzo IP esterno (precedentemente load balancer) che fornisce un gateway alle funzionalità della macchina virtuale. Fornisci questo indirizzo durante la configurazione iniziale. L'indirizzo viene utilizzato dall'interfaccia ONTAP tools Manager e per accedere direttamente alla pagina della documentazione Swagger e alla REST API.
