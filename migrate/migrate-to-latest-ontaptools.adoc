...
NOTE: Si consiglia di eseguire la migrazione dei tool ONTAP per la configurazione di VMware vSphere 9.xx solo se la configurazione utilizza solo la funzionalità provider VASA.


NOTE: Dopo la migrazione da strumenti ONTAP per VMware vSpher 9.x a 10,3, i datastore vVol con protocollo NVMe/FC non funzionano perché l'attrezzo ONTAP 10,3 supporta solo datastore NVMe-of con datastore VMFS.

*A proposito di questa attività*

La migrazione è supportata dai tool ONTAP per le release VMware vSphere 9.12D1 e 9.13D2 alla release 10,3.
...
. Aggiungere l'istanza di vCenter Server che si desidera migrare agli strumenti ONTAP per VMware vSphere 10,3. Vedere link:../configure/add-vcenter.html["Aggiungere istanze di vCenter Server"]
...
.. Fare clic sul menu puntini di sospensione, quindi su *Reimposta configurazione*
.. Fare clic sul menu puntini di sospensione e selezionare *Elimina*
+
...
Dopo aver eseguito la migrazione dalle vecchie versioni degli strumenti ONTAP per VMware vSphere alla versione 10,3, eseguire nuovamente la scansione degli adattatori SRA per verificare che i dettagli siano aggiornati nella pagina adattatori di replica dello storage per il ripristino dei siti live di VMware.
