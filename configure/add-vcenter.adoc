...
Quando si registra un'istanza del server vCenter con ONTAP tools Manager, assicurarsi che il certificato del server vCenter includa un'estensione Subject Alternative Name (SAN) valida. L'estensione SAN deve contenere sia voci DNS che indirizzi IP. Ad esempio:
...
----
Se il certificato del server vCenter non include un'estensione SAN, o se l'estensione SAN non contiene i valori DNS o di indirizzo IP corretti, le operazioni degli ONTAP tools potrebbero non riuscire a causa di errori di convalida del certificato.
