...
Sono supportati gli snapshot di macchine virtuali senza memoria. Se una macchina virtuale ha uno snapshot con memoria, la macchina virtuale non può essere protetta.

Non è possibile proteggere le macchine virtuali non protette che contengono snapshot di memoria. Elimina gli snapshot di memoria prima di abilitare la protezione per la macchina virtuale.

Per una VM Windows con tipo di storage ASA r2, gli snapshot della macchina virtuale sono di sola lettura. Quando si accende la VM, VASA Provider crea una LUN dallo snapshot di sola lettura e abilita le IOPS. Quando si spegne la VM, VASA Provider elimina la LUN e disabilita le IOPS.
