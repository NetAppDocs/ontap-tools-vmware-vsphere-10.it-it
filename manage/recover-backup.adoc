...
summary: Ripristina gli ONTAP tools dal backup e comprendi il comportamento a RPO zero e quasi zero per release. 
---
...
[role="lead"]
Puoi recuperare l'installazione di ONTAP tools for VMware vSphere ripristinandola da un file di backup archiviato nel datastore. A partire da ONTAP tools for VMware vSphere 10.5, la funzionalità di backup è abilitata per impostazione predefinita, quindi è disponibile un backup recente se hai bisogno di ridistribuire o recuperare l'appliance.
...
. Individua la nuova macchina virtuale ONTAP tools appena distribuita e avvia la console web.
...
----
+
I log vengono registrati nel file _/var/log/otvng/post-deploy-upgrade.log_.
